...
// to gRPC service.
func StartHealthCheck(conn *grpc.ClientConn, balancer grpc.Balancer, svc string) chan<- struct{} {
			...
			case <-ticker.C:
				...
				for i := 0; i < size; i++ {
					...
					if _, err := cli.Check(ctx, &req); err != nil {
						healthCheckCounts.WithLabelValues(grpc.Code(err).String(), svc).Inc()
						if !IsSafeError(err) {
