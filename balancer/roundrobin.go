...
type roundRobin struct {
	...
	// TODO(fuyc): remove this patch after skylb diag protocol is implemented.
	debugPrintStopCh  chan struct{}
	debugPrintStopped bool
}
...
func (rr *roundRobin) Get(ctx context.Context, opts grpc.BalancerGetOptions) (addr grpc.Address, put func(), err error) {
	...
	if health.IsHealthCheck(ctx) {
		return rr.getNextHealthAddr(ctx, opts)
	}
	return rr.getNextAddr(ctx, opts)
}

func (rr *roundRobin) getNextAddr(ctx context.Context, opts grpc.BalancerGetOptions) (addr grpc.Address, put func(), err error) {
	var ch chan struct{}
	if len(rr.addrs) > 0 {
...
// getNextHealthAddr is a dup of getNextAddr, with all occurrences of "rr.next"
// replaced with "rr.nextHealth".
func (rr *roundRobin) getNextHealthAddr(ctx context.Context, opts grpc.BalancerGetOptions) (addr grpc.Address, put func(), err error) {
	var ch chan struct{}
	if len(rr.addrs) > 0 {
		if rr.nextHealth >= len(rr.addrs) {
			rr.nextHealth = 0
		}
		next := rr.nextHealth
		for {
	...
	if !opts.BlockingWait {
		...
		// Returns the next addr on rr.addrs for failfast RPCs.
		addr = rr.addrs[rr.nextHealth].addr
		rr.nextHealth++
		rr.mu.Unlock()
		return
	}
...
func (rr *roundRobin) StartDebugPrint(interval time.Duration) {
	rr.mu.Lock()
	defer rr.mu.Unlock()

	rr.debugPrintStopCh = make(chan struct{})

	ticker := time.NewTicker(interval)
	go func() {
		for {
			select {
			case <-rr.debugPrintStopCh:
...
func (rr *roundRobin) StopDebugPrint() {
	rr.mu.Lock()
	defer rr.mu.Unlock()

	if !rr.debugPrintStopped {
		close(rr.debugPrintStopCh)
	}
	rr.debugPrintStopped = true
}
