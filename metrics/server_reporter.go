...
var (
	serverStartedCounter = prom.NewCounterVec(
		...
		}, []string{"grpc_type", "grpc_service", "grpc_method", CALLER_SERVICE})
	// "grpc_service" now means the callee service (current service itself)
	...
	serverHandledCounter = prom.NewCounterVec(
		...
		}, []string{"grpc_type", "grpc_service", "grpc_method", "grpc_code", CALLER_SERVICE})
...
func newServerReporter(spec *pb.ServiceSpec, rpcType grpcType, fullMethod string, callerServiceName string) *serverReporter {
	r := &serverReporter{
		rpcType:           rpcType,
		spec:              spec,
	...
	r.serviceName = spec.ServiceName
	serverStartedCounter.WithLabelValues(string(r.rpcType), r.serviceName, r.methodName, callerServiceName).Inc()
	return r
}

func (r *serverReporter) ReceivedMessage() {
	serverStreamMsgReceived.WithLabelValues(string(r.rpcType), r.serviceName, r.methodName, r.callerServiceName).Inc()
}

func (r *serverReporter) SentMessage() {
	serverStreamMsgSent.WithLabelValues(string(r.rpcType), r.serviceName, r.methodName, r.callerServiceName).Inc()
}

func (r *serverReporter) Handled(code codes.Code) {
	serverHandledCounter.WithLabelValues(string(r.rpcType), r.serviceName, r.methodName, code.String(), r.callerServiceName).Inc()
	if serverHandledHistogramEnabled {
